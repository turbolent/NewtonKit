...
public struct RawDockPacket {
    ...
    public init(data: Data) throws {
        ...
        let headerEndIndex = headerStartIndex.advanced(by: 2 * RawDockPacket.boundary)
        let header = data.subdata(in: headerStartIndex..<headerEndIndex)
        guard header == RawDockPacket.header else {
            throw DecodingError.invalidHeader
        }

        // decode command
        let commandStartIndex = headerEndIndex
        let commandEndIndex = commandStartIndex.advanced(by: RawDockPacket.boundary)
        ...
        let lengthEndIndex = lengthStartIndex.advanced(by: RawDockPacket.boundary)
    ...
    public func encode() -> Data {
        var result = Data()
        result.append(RawDockPacket.header)
...
extension RawDockPacket: Equatable {

    public static func == (lhs: RawDockPacket, rhs: RawDockPacket) -> Bool {
